...
Namespace Chessie.CSharp.Test
    ...
    Class Person
        ...
        Public Property Clothes() As List(Of String)
            ...
            End Set
        End Property
        Private m_Clothes As List(Of String)
    ...
    Class Test1
        <Test>
        Public Sub Part1()
            Dim Dave = New Person(Gender.Male, 41, New List(Of String)() From {
                "Tie",
                "Jeans"
            }, Sobriety.Sober)
            ...
            Dim Ken = New Person(Gender.Male, 28, New List(Of String)() From {
                "Tie",
                "Shirt"
            }, Sobriety.Tipsy)
            ...
            Dim Ruby = New Person(Gender.Female, 25, New List(Of String)() From {
                "High heels"
            }, Sobriety.Tipsy)
            Dim costRuby = ClubbedToDeath.CostToEnter(Ruby)
            costRuby.Match(Sub(x, msgs)
            ...
            costKenUnconscious.Match(Sub(x, msgs)
                                         Assert.Fail()
    ...
    Class Test2
        <Test>
        Public Sub Part2()
            Dim daveParalytic = New Person(age:=41, clothes:=New List(Of String)() From {
                "Tie",
                "Shirt"
            }, gender:=Gender.Male, sobriety:=Sobriety.Paralytic)

            Dim costDaveParalytic = ClubTropicana.CostToEnter(daveParalytic)

            costDaveParalytic.Match(ifSuccess:=Sub(x, msgs) Assert.Fail(), ifFailure:=Sub(errs) Assert.That(errs.ToList(), [Is].EquivalentTo({"Too old!", "Sober up!"})))
        End Sub
    End Class
    ...
    Class GayBar
        ...
        Public Shared Function CostToEnter(p As Person) As Result(Of Decimal, String)
            Return New List(Of Func(Of Person, Result(Of Person, String)))() From {
    ...
    Class Test3
        <Test>
        Public Sub Part3()
            Dim person = New Person(gender:=Gender.Male, age:=59, clothes:=New List(Of String)() From {
                "Jeans"
            }, sobriety:=Sobriety.Paralytic)
            Dim cost = GayBar.CostToEnter(person)
            cost.Match(ifSuccess:=Sub(x, msgs) Assert.Fail(), ifFailure:=Sub(errs) Assert.That(errs, [Is].EquivalentTo({"Too old!", "Smarten up!", "Sober up!"})))
        End Sub
    End Class
End Namespace